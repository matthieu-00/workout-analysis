...
export default function SuggestionsPanel({ suggestions }: SuggestionsPanelProps) {
  ...
  if (suggestions.length === 0) {
    return (
      <div className="bg-white rounded-lg shadow-md border border-gray-200 p-4 md:p-6 h-full flex flex-col">
          ...
          return (
            ...
            >
              <button
                ...
                className="w-full p-4 bg-purple-50 hover:bg-purple-100 active:scale-[0.98] transition flex justify-between items-center cursor-pointer border-b border-purple-200"
              >
                    ...
                    {hasMore && (
                      <>
                        ...
                        <button
                          ...
                          className="w-full text-sm text-purple-600 hover:text-purple-700 active:scale-95 font-semibold py-3 flex items-center justify-center gap-2 transition cursor-pointer"
                        >
                          {showAllExercises ? (
                            <>
                              <ChevronUp size={16} />
      ...
      <div className="flex items-start justify-between p-4">
        <div className="flex-1">
          <div className="flex items-center gap-3">
            <h5 className="font-semibold text-black">{exercise.name}</h5>
          ...
          <div className="flex flex-wrap gap-3 mt-4">
            <span className="text-xs px-3 py-1.5 bg-purple-100 text-purple-800 rounded font-semibold">
              {exercise.category}
            </span>
            {exercise.equipment && (
              <span className="text-xs px-3 py-1.5 bg-gray-100 text-gray-700 rounded font-semibold">
                {exercise.equipment}
              </span>
            )}
          </div>
        ...
        {onAdd && (
          <button
            ...
            className="ml-4 p-3 bg-green-500 text-white hover:bg-green-600 active:scale-95 rounded transition cursor-pointer"
